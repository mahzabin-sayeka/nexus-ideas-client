  ...
  return (
    <nav className="w-full bg-white dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 transition-colors shadow-sm sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <div className="hidden md:flex gap-8 font-medium text-slate-700 dark:text-slate-300 items-center">
          <Link className="hover:text-emerald-600 transition-colors" href="/">Home</Link>
          <Link className="hover:text-emerald-600 transition-colors" href="/ideas">Ideas</Link>
          {isLoggedIn && (
            <>
              <Link className="hover:text-emerald-600 transition-colors" href="/add-idea">Add Idea</Link>
              <Link className="hover:text-emerald-600 transition-colors" href="/my-ideas">My Ideas</Link>
            </>
          )}
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="hidden md:flex items-center gap-4">
            {isLoggedIn ? (
              <div className="flex items-center gap-2">
                <div className="w-9 h-9 rounded-full bg-emerald-600 border-2 border-emerald-200"></div>
                <span className="font-semibold dark:text-slate-100">{session?.user?.name?.slice(0, 2).toUpperCase()}</span>
              </div>
            ) : (
              <div className="flex gap-4">
                <Link className="px-4 py-2 hover:text-emerald-600 dark:text-slate-300 transition-colors" href="/login">Login</Link>
                <Link className="bg-slate-900 dark:bg-emerald-600 text-white px-5 py-2 rounded-full hover:bg-emerald-600 transition-all" href="/register">Register</Link>
              </div>
            )}
          </div>
      ...
      {isOpen && (
        <div className="md:hidden bg-white dark:bg-slate-950 border-t p-6 flex flex-col gap-4 text-center">
          ...
          {isLoggedIn ? (
            ...
            <>
              <Link href="/login" onClick={() => setIsOpen(false)}>Login</Link>
              <Link className="bg-emerald-600 text-white py-2 rounded-full" href="/register" onClick={() => setIsOpen(false)}>Register</Link>
            </>
          )}
        </div>
      )}
